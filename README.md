...
## ✨ Ver.2.6.1 직접 업로드 탭에 URL 입력 기능 추가! (2025.12.31) ⭐ NEW!

### 📹 3가지 영상 업로드 방식 지원! ⭐ NEW!
차시에 영상을 등록할 때 **YouTube URL**, **파일 업로드**, **URL 입력** 3가지 방식을 선택할 수 있습니다!
...
2. **영상 설정** 탭에서 방식 선택:
   ...
   - **직접 업로드 탭**:
     - **파일 업로드 버튼**: 파일 선택 또는 드래그 앤 드롭
     - **URL 입력 버튼**: api.video URL 또는 직접 영상 URL 입력 ⭐ NEW!
3. 업로드 완료 후 저장
...
- ✅ **직접 업로드 탭에 URL 입력 기능 추가** 🎉
  - 파일 업로드 ↔ URL 입력 토글 버튼
  - api.video URL 즉시 등록 (Video ID 추출)
  - 직접 영상 URL 자동 업로드
  - URL 타입 자동 감지 및 처리
  - 업로드 진행률 표시
  - 사용자 친화적 UI
- ✅ **3가지 영상 업로드 방식 완성**
  ...
  - URL 입력 (신규)
- ✅ **문서 작성**
  - `URL_UPLOAD_GUIDE.md` - URL 업로드 완전 가이드
